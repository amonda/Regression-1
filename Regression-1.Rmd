...
summary(Galton) #generating uniform set based on the random number ranging between 0 and 1. length command asys make 905. this steps is creating a new variable and adding it to that dataset
Galton.train <- subset(Galton, group <= 0.90) #if number less than .9 they get assignmed to traning
Galton.test <- subset(Galton, group > 0.90) #if number is more than .9 they get assigned to the test group for the model
...
plot(child ~ parent, data = Galton.test) # ~ is a command "y~x" before ~ is outcome and after is the predictor so before ~ is y axis and after is x axis
```
...
reg1 <- lm(child ~ parent, data = Galton.train) #assigns the info from running the regression as a new variable called reg1 so you can do stuff with this you can also just run the command by itself but if you assign it to make an object or list it contains more information about the model
summary(reg1) #lm stands for the command for linear model lm(formula for y~x format)
```
...
```{r}
# run a prediction showing if you only use the parent heights in your model how closely can you predict the child height to see if your model works
#get predicted values in the training and testing dataset
Galton.train$pred.child <- predict(reg1, newdata = Galton.train) #using info in reg 1 to sa use that data to predict a new variable
...
residualPlots(reg1)
#shows if there are outliers. if p value is significant there are outliers in the set, if not your data does not have outliers?
```
...
****residuals should graph out around 0 so it is helpful to put 0 line on your graph
```{r}
#set up the 0 line, without this the graph looks strange
x0 <- 64
parent <- seq(x0, 73, 0.01) #creating a new variable that wasn't in the data set could've called it anything she wanted the point was to put a line on the graph
resid <- rep(0, length(parent))

#plot residual versus X
plot(resid~parent, data = Galton.test) #laying the plot of the data we want over the line she created with the variable creation
#overlay the zero line
par(new=TRUE) #command for overlay
plot(resid~parent, pch = 16, cex = .1, axes = FALSE, labels = FALSE) #creates the plot
...
We can formally test this assumption, but we can also do a qq-plot which will give us a visual representation of the same. But with 900 values any deviation will give you a significant result. So we use qqplot to geta studentized residual which divides it by the standard error. ranks them and assigns percentile values and graphs it. if there is a normal distriution it should follow a straight line. 
We get this plot as follows:
...
How do we make heads or tails out of the plots above? One way is with an influence plot. Which aggregates the plots from above into one. 
```{r}
#make influence plot
...
ncvTest(prestige.mod1) #tests for non constant variance. All biomarkers fail this test. since p is big its a homoskedastic set
```
